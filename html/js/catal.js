document.addEventListener("DOMContentLoaded", () => {
  ...
  const fetchOutilDetails = async (id, action = "reserver") => {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des détails de l'outil:",
        error
      );
      showMessage(
        "Une erreur est survenue lors du chargement des détails de l'outil.",
        "error"
      );
    }
  };
  ...
  const showModal = (outil) => {
    ...
    outilTitre.innerHTML = `
      ...
      <p><strong>Catégorie:</strong> ${
        outilSelectionne.categorie || "Non catégorisé"
      }</p>
      <p><strong>Prix:</strong> ${prixJournalier.toFixed(2)} €/jour</p>
      <div class="dates-reservation">
        <div class="date-field">
          <label for="date-debut">Date de début:</label>
          <input type="date" id="date-debut" value="${datePicker.value}" min="${
      datePicker.value
    }">
        </div>
        <div class="date-field">
          <label for="date-fin">Date de fin:</label>
          <input type="date" id="date-fin" value="${formattedEndDate}" min="${
      datePicker.value
    }">
        </div>
      </div>
  ...
  const addToPanier = async (outilId) => {
    ...
    try {
      const user = JSON.parse(localStorage.getItem("user"));
      const userId =
        user && user.id ? user.id : "a1a1a1a1-a1a1-a1a1-a1a1-a1a1a1a1a1a1";
      ...
      const response = await fetch(
        `${API_BASE_URL}/users/${userId}/panier/outils/${outilId}`,
        {
          method: "POST",
          headers: headers,
        }
      ...
      if (modal.style.display === "flex") {
        ...
        if (dateDebut && dateFin) {
          ...
          localStorage.setItem(
            "reservation_total",
            document.querySelector(".prix-total span").textContent
          );
        }
      }
